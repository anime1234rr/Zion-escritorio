...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  onWindowHidden: (callback: () => void): (() => void) => {
    const listener = () => callback()
    ipcRenderer.on('zion-window-hidden', listener)
    return () => ipcRenderer.removeListener('zion-window-hidden', listener)
  },
  readyToQuit: (): void => {
  ...
  unlockInspect: (payload: {
    supabaseUrl: string
    anonKey: string
    accessToken: string
  }): Promise<boolean> => ipcRenderer.invoke('zion:unlock-inspect', payload),
  writeClipboardImage: (dataUrl: string): Promise<void> =>
    ipcRenderer.invoke('zion:write-clipboard-image', dataUrl),
  ...
  media: {
    save: (bytes: Uint8Array, ext: string): Promise<string> =>
      ipcRenderer.invoke('zion:media-save', { bytes, ext }),
    remove: (id: string): Promise<void> => ipcRenderer.invoke('zion:media-delete', id),
    usage: (): Promise<{ count: number; bytes: number }> =>
      ipcRenderer.invoke('zion:media-usage'),
    openFolder: (): void => {
      ipcRenderer.send('zion:media-open-folder')
    },
  },
})
