...
import { mensajeMeMenciona, renderMessageContent } from '@/lib/render-message-content'
...
import { claveDraftCanal } from '@/lib/drafts'
...
import { alternarMensajeGuardado, useMensajeGuardado } from '@/lib/saved-messages'
...
const MessageRow = memo(function MessageRow({
  ...
  const isSaved = useMensajeGuardado(message.id)
  const miUsername = mentionableMembers.find((m) => m.id === currentUserId)?.username
  const mentionsMe =
    !isOwnMessage && message.content ? mensajeMeMenciona(message.content, miUsername) : false
  ...
  const actions = useMessageActions({
    ...
    onCopyLink: () =>
      writeClipboard(buildChannelMessageLink(server.id, channelId, message.id)),
    ...
    isSaved,
    onToggleSave: () => {
      void alternarMensajeGuardado(message, {
        scope: 'canal',
        serverId: server.id,
        serverName: server.name,
        channelId,
        channelName,
        link: buildChannelMessageLink(server.id, channelId, message.id),
      })
    },
  })
      ...
      <ContextMenuTrigger asChild>
        <div
          ...
          className={cn(
            ...
            highlighted
              ? 'bg-primary/10'
              : mentionsMe && 'border-l-2 border-primary bg-primary/[0.06] pl-2'
          )}
        >
          {message.pinned && (
                  ...
                  <div className="max-h-80 w-fit max-w-full overflow-hidden rounded-lg border border-border sm:max-w-sm">
                    {message.attachment.type === 'image' ? (
                      <img
                        src={message.attachment.url}
                        alt=""
                        onClick={() => onViewAttachment(message)}
          ...
          {canReact && (
            <Popover>
              ...
              <PopoverContent side="top" align="end" collisionPadding={16} className="w-auto p-0">
...
export function ChatPrincipal({
  ...
  useDraftSync(claveDraftCanal(channel.id), draft, setDraft)
  ...
  const [frozenLastRead, setFrozenLastRead] = useState<string | null>(lastReadAt ?? null)
  const [seenChannelForRead, setSeenChannelForRead] = useState(channel.id)
  if (seenChannelForRead !== channel.id) {
    setSeenChannelForRead(channel.id)
    setFrozenLastRead(lastReadAt ?? null)
  }

  const unreadDividerGroupIndex = useMemo(() => {
  ...
  const [mentionableRoles, setMentionableRoles] = useState<ServerRole[]>([])
  ...
  useEffect(() => {
    ...
    listarRolesDeServidor(server.id)
      .then((data) => !cancelado && setMentionableRoles(data))
      .catch(() => {})
    return () => {
      cancelado = true
    }
  ...
  const mentionAutocompleteQuery = useMemo(() => {
    const match = /(^|\s)@([a-zA-Z0-9_]{0,32})$/.exec(draft)
    return match ? match[2] : null
  }, [draft])
  ...
  const handleCreateThreadFromMessage = useCallback((message: ChatMessage) => {
  ...
  function handleJumpToMessage(messageId: string) {
    ...
    requestAnimationFrame(() => setLocalHighlightId(messageId))
  }
  ...
  return (
    ...
    >
      ...
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1">
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">Mensajes guardados</TooltipContent>
          </Tooltip>
      ...
      <div
        ...
        className="relative flex flex-1 flex-col overflow-y-auto px-2"
      >
        <div className="min-h-0 shrink grow" aria-hidden />
        <div
          ref={listRef}
          className="shrink-0"
        ...
        >
          {virtualItems.map((virtualRow) => {
            ...
            return (
              ...
              >
                {virtualRow.index === unreadDividerGroupIndex && (
                  <div className="mx-2 mb-1 flex items-center gap-2" aria-label="Mensajes nuevos">
                    <span className="h-px flex-1 bg-destructive/60" />
                    <span className="rounded-full bg-destructive/15 px-2 py-0.5 text-[11px] font-semibold tracking-wide text-destructive uppercase">
                      Nuevos mensajes
                    </span>
                    <span className="h-px flex-1 bg-destructive/60" />
                  </div>
                )}
                <div className="group/row flex items-start gap-3 rounded-md px-2 py-0.5 hover:bg-muted/30">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline gap-2">
                  ...
                  {!group.author.isWebhook && !group.author.isBot && (
                    <UserBadges userId={group.author.id} />
                  )}
                  {group.author.isBot && (
                ...
                {group.items.map((message) => (
                  <MessageRow
                    ...
                    onViewAttachment={setViewerMessage}
        ...
        >
          ...
          {stickers.length > 0 && (
            <Popover open={stickerPopoverOpen} onOpenChange={setStickerPopoverOpen}>
              ...
              className="grid w-64 grid-cols-3 gap-2 overflow-y-auto p-2"
            >
      ...
      <SavedMessagesDialog
        ...
        onNavigateToLink={(link) => onNavigateToLink?.(link)}
      />
      ...
      {viewerMessage?.attachment && (
        <Suspense fallback={null}>
          <MediaViewerDialog
            key={viewerMessage.id}
            open
            onOpenChange={(next) => !next && setViewerMessage(null)}
            attachment={viewerMessage.attachment}
            onForward={() => onForwardMessage(viewerMessage)}
          />
        </Suspense>
      )}
    </section>
