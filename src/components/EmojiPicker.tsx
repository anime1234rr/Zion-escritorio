...
export function EmojiPicker({ onSelect, className, customEmojis }: EmojiPickerProps) {
  ...
  return (
    <div
      style={{ height: 'min(30rem, var(--radix-popover-content-available-height, 30rem))' }}
      className={cn('flex w-72 flex-col overflow-hidden', className)}
    >
                ...
                {category.emojis.map((emoji) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex size-9 items-center justify-center rounded-md text-xl outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring/50"
                    >
                      {customUrl ? (
                        <img src={customUrl} alt={emoji.char} className="size-6 object-contain" />
                      ) : (
                        emoji.char
                      )}
                    </button>
