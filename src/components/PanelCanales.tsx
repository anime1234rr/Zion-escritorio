...
} from '@/components/server-settings/ServerSettingsPanel'
    ...
    <>
      ...
      >
        ...
        {channel.mentionCount ? (
          <span className="ml-auto flex h-4 min-w-4 shrink-0 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-semibold text-primary-foreground">
...
interface PanelCanalesProps {
  ...
  onExitPreview?: () => void
  pendingAction?: { kind: 'settings' | 'invite'; section?: ServerSettingsSectionId } | null
}
...
export function PanelCanales({
  ...
  const [settingsSection, setSettingsSection] = useState<ServerSettingsSectionId>('general')
  ...
  const [seenAction, setSeenAction] = useState(pendingAction ?? null)
  if (pendingAction && pendingAction !== seenAction) {
    setSeenAction(pendingAction)
    if (pendingAction.kind === 'settings') {
      setSettingsSection(pendingAction.section ?? 'general')
