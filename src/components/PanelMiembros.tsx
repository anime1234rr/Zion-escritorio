...
export function PanelMiembros({ server, currentUserId, onMessageUser }: PanelMiembrosProps) {
  ...
  const conectados = useMemo(() => members.filter((m) => m.user.status !== 'offline'), [members])
  const desconectados = useMemo(() => members.filter((m) => m.user.status === 'offline'), [members])
              ...
              >
                ...
                >
                  ...
                  <span className="flex min-w-0 items-center gap-1 truncate text-sm text-sidebar-foreground">
                    ...
                    {!member.user.isBot && <UserBadges userId={member.user.id} size="size-3" />}
                    {voice &&
                      (voice.muted ? (
