...
import { claveDraftDM } from '@/lib/drafts'
  ...
  const actions = useMessageActions({
    ...
    onToggleSave: () => {
      void alternarMensajeGuardado(message, {
        scope: 'dm',
        conversationId,
        link: buildDMMessageLink(conversationId, message.id),
      })
    },
  })
...
export function DMChatPrincipal({
  ...
  useDraftSync(claveDraftDM(conversationId), draft, setDraft)
          ...
          >
            {virtualItems.map((virtualRow) => {
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3 rounded-md px-2 py-0.5 hover:bg-muted/30">
