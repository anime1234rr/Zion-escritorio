...
function formatFechaBaneo(iso: string): string {
...
export function PersonasSection({ server, currentUserId }: PersonasSectionProps) {
  ...
  async function handleToggleRole(member: ServerMember, role: ServerRole) {
    if (role.esRolBase) return
    const tiene = member.roles.some((r) => r.id === role.id)
    setActionError(null)
    const previous = members
    setMembers((prev) =>
      prev.map((m) => {
        if (m.membershipId !== member.membershipId) return m
        const roles = tiene
          ? m.roles.filter((r) => r.id !== role.id)
          : [...m.roles, role].sort((a, b) => a.posicion - b.posicion)
        return { ...m, roles }
      })
    )
    try {
      if (tiene) await quitarRolMiembro(member.membershipId, role.id)
      else await agregarRolMiembro(member.membershipId, role.id)
    } catch (err) {
      setMembers(previous)
    ...
    <div className="max-w-2xl">
          ...
          {sortedMembers.map((member) => {
            ...
            return (
              ...
              >
                ...
                {(() => {
                  const principal = rolPrincipal(member)
                  const extra = member.roles.length - 1
                  const resumen = (
                    <>
                      <span
                        className="size-2 rounded-full"
                        style={{ backgroundColor: principal?.color ?? '#9ca3af' }}
                      />
                      {principal?.nombre ?? 'Sin rol'}
                      {extra > 0 && (
      ...
      {banTarget && (
        <BanMemberDialog
          ...
          onConfirm={async (razon, minutos) => {
            await banearMiembro(banTarget.membershipId, razon, minutos ?? undefined)
            setBanTarget(null)
          }}
        />
      )}
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-1.5">
                  ...
                  >
                    {banned.expiresAt ? `Hasta ${formatFechaBaneo(banned.expiresAt)}` : 'Permanente'}
                  </span>
                </span>
                {banned.reason && (
