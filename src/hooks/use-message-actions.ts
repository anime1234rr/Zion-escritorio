import { Bookmark, BookmarkX, Copy, Files, Forward, Link, MessagesSquare, Pin, PinOff, Pencil, Reply, Trash2 } from 'lucide-react'
...
export function useMessageActions({
  ...
  if (onToggleSave) {
    actions.push(
      isSaved
        ? { key: 'unsave', label: 'Quitar de guardados', icon: BookmarkX, onSelect: onToggleSave }
        : { key: 'save', label: 'Guardar mensaje', icon: Bookmark, onSelect: onToggleSave }
    )
  }
