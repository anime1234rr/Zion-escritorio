...
import { onBeforeQuit, onWindowHidden, readyToQuit, selectScreenSource } from '@/lib/electron-bridge'
...
onWindowHidden(() => {
  if (state.connectedChannelId) {
    leaveVoiceChannel().catch((err) => console.error('No se pudo salir de voz al minimizar', err))
  }
})
...
onBeforeQuit(() => {
  ...
  leaveVoiceChannel()
    .catch((err) => console.error('No se pudo salir de voz al cerrar', err))
    .finally(() => readyToQuit())
})
