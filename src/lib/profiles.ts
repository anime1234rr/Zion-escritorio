...
import { createCache } from '@/lib/internal/local-cache'
import { measureNetworkCall } from '@/lib/internal/perf-metrics'
...
export async function actualizarPreferenciasNotificacionSeguridad(
  userId: string,
  cambios: Partial<SecurityNotificationPrefs>
): Promise<void> {
  const patch: Record<string, boolean> = {}
  for (const clave of Object.keys(cambios) as (keyof SecurityNotificationPrefs)[]) {
    const valor = cambios[clave]
    if (valor !== undefined) patch[COLUMNA_POR_CLAVE[clave]] = valor
  }
  if (Object.keys(patch).length === 0) return

  const { error } = await supabase.from('perfiles').update(patch).eq('id', userId)
  if (error) throw error
}
...
export async function actualizarAvatar(userId: string, avatarUrl: string) {
  ...
  if (error) throw error
  perfilPublicoCache.delete(userId)
}
...
const perfilPublicoCache = createCache<PublicProfile>({ ttlMs: 60_000, maxEntries: 300 })

export async function obtenerPerfilPublico(userId: string): Promise<PublicProfile> {
  return perfilPublicoCache.getOrSet(userId, () =>
    measureNetworkCall('obtenerPerfilPublico', async () => {
      const { data, error } = await supabase
        .from('perfiles')
        .select('*')
        .eq('id', userId)
