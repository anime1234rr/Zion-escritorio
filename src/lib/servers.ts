...
export async function actualizarServidor(
  ...
  if (cambios.descripcion !== undefined)
    patch.descripcion = cambios.descripcion?.trim() ? cambios.descripcion.trim() : null
...
export async function crearServidor(
  ...
  if (error) {
    const aviso = mensajeErrorSancion(error)
    throw aviso ? new Error(aviso) : error
  }
...
export async function unirseAServidor(codigoInvitacion: string): Promise<ServerItem> {
  ...
  if (error) {
    const aviso = mensajeErrorAutomod(error) ?? mensajeErrorSancion(error)
    throw aviso ? new Error(aviso) : error
  }
...
export async function abandonarServidor(servidorId: string): Promise<void> {
  const {
    data: { user },
